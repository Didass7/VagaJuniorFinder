...
class IndeedScraper(BaseScraper):
    ...
    def _fetch_via_http_session(self, queries: List[str]) -> List[Job]:
        """Attempts direct HTTP scraping with realistic rotated browser headers and cookies."""
        jobs: List[Job] = []
        ...
        if config.indeed_cookies:
            headers["Cookie"] = config.indeed_cookies
    ...
    def fetch(self) -> List[Job]:
        ...
        # Tier 1: Undetected Browser / SeleniumBase UC
        try:
        ...
        # Tier 2: Direct HTTP session / cookie scraping (if browser yielded 0)
        if not all_jobs:
            try:
                ...
                logger.debug(f"[Indeed Portal] HTTP strategy error: {e}")
